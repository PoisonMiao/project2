...
public abstract class AbstractConnectionController implements ConnectionController, GearmanConnectionHandler<Object>, GearmanCallbackHandler<GearmanServer, ConnectCallbackResult> {

	private final AbstractJobServerPool<?> sc;

	/** The key mapping to this object in the connMap */
	private final GearmanServer key;
	...
	protected AbstractConnectionController(AbstractJobServerPool<?> sc, GearmanServer key) {
		this.key = key;
		this.sc = sc;
	...
	public void onStatusReceived(GearmanPacket packet) {
		...
		final boolean isKnown		= b_isKnown.length > 0 && (b_isKnown[0] == '1');
		final boolean isRunning		= b_isRunning.length > 0 && (b_isRunning[0] == '1');
			...
			if(this.state.equals(ControllerState.CONNECTING)) {
				...
				if(this.pendingJobStatus!=null && !this.pendingJobStatus.isEmpty()) {
					...
					for(Entry<ByteArray, TaskJoin<GearmanJobStatus>> status : this.pendingJobStatus.entrySet()) {
						...
						this.sendPacket(GearmanPacket.createGET_STATUS(jobHandle.getBytes()), (data, result) -> {
	...
	@Override
	public void onComplete(GearmanServer data, ConnectCallbackResult result) {
	...
	public final GearmanServer getKey() {
		return this.key;
	}
	...
	public void sendPacket(GearmanPacket packet, GearmanCallbackHandler<GearmanPacket, SendCallbackResult> callback, int connectionId) {
	...
	private void completeJobStatus(ByteArray jobHandle, boolean isKnown, boolean isRunning, long numerator, long denominator) {
		synchronized(this.lock) {
			final TaskJoin<GearmanJobStatus> taskJoin = getGearmanJobStatusTaskJoin(jobHandle);

			taskJoin.setValue(new GearmanJobStatusImpl(isKnown, isRunning, numerator, denominator));
		}
	}

	private void completeJobStatus(GearmanJobStatusType type, ByteArray jobHandle, boolean isKnown, boolean isRunning, long numerator, long denominator) {
		synchronized(this.lock) {
			final TaskJoin<GearmanJobStatus> taskJoin = getGearmanJobStatusTaskJoin(jobHandle);
	...
	private TaskJoin<GearmanJobStatus> getGearmanJobStatusTaskJoin(ByteArray jobHandle) {
	...
	private void dealingConnecting() {
		if (this.pendingJobStatus != null) {
