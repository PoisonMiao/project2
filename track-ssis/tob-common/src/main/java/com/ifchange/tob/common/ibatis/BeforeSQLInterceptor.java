...
import com.ifchange.tob.common.core.BaseEntity;
...
public class BeforeSQLInterceptor implements Interceptor {
    ...
    private Object invokeBeforeBoundSql(Invocation invocation) throws Exception {
        ...
        if("true".equalsIgnoreCase(showSQL)) {
            ...
            System.out.println(String.format("@db %s @cmd %s @sql %s ", db, ms.getSqlCommandType(), sql));
            System.out.println(String.format("@sql-param %s", JsonHelper.toJSONString(args[1])));
        }
