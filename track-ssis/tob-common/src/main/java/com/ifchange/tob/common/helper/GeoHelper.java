...
public final class GeoHelper {
    ...
    public static List<String> nearNine(final BigDecimal lat, final BigDecimal lon) {
        ...
        BigDecimal upLng = MathHelper.nvl(lon).subtract(MIN_LAT_LNG.get("MIN_LNG"));
        BigDecimal downLng = MathHelper.nvl(lon).add(MIN_LAT_LNG.get("MIN_LNG"));

        List<String> base32For9 = Lists.newArrayList();

        //左侧从上到下 3个
        String leftUp = getGeoHashBase32(leftLat, upLng);
        if (!(leftUp == null || StringHelper.EMPTY.equals(leftUp))) {
            base32For9.add(leftUp);
        }
        String leftMid = getGeoHashBase32(leftLat, lat);
        if (!(leftMid == null || StringHelper.EMPTY.equals(leftMid))) {
            base32For9.add(leftMid);
        }
        String leftDown = getGeoHashBase32(leftLat, downLng);
        if (!(leftDown == null || StringHelper.EMPTY.equals(leftDown))) {
            base32For9.add(leftDown);
        }

        //中间从上到下 3个
        String midUp = getGeoHashBase32(lat, upLng);
        if (!(midUp == null || StringHelper.EMPTY.equals(midUp))) {
            base32For9.add(midUp);
        }
        String midMid = getGeoHashBase32(lat, lon);
        if (!(midMid == null || StringHelper.EMPTY.equals(midMid))) {
            base32For9.add(midMid);
        }
        String midDown = getGeoHashBase32(lat, downLng);
        if (!(midDown == null || StringHelper.EMPTY.equals(midDown))) {
            base32For9.add(midDown);
        }

        //右侧从上到下 3个
        String rightUp = getGeoHashBase32(rightLat, upLng);
        if (!(rightUp == null || StringHelper.EMPTY.equals(rightUp))) {
            base32For9.add(rightUp);
        }
        String rightMid = getGeoHashBase32(rightLat, lon);
        if (!(rightMid == null || StringHelper.EMPTY.equals(rightMid))) {
            base32For9.add(rightMid);
        }
        String rightDown = getGeoHashBase32(rightLat, downLng);
        if (!(rightDown == null || StringHelper.EMPTY.equals(rightDown))) {
     ...
     */
    public static BigDecimal distance(final BigDecimal lonA, BigDecimal latA, final BigDecimal lonB, BigDecimal latB) {
        latA = rad(latA);
        latB = rad(latB);
        BigDecimal aDiff = MathHelper.nvl(latA).subtract(latB);
        BigDecimal nDif = rad(MathHelper.nvl(lonA).subtract(lonB));
     ...
     * 对经纬度进行HASH编码
     */
    public static String encode(final BigDecimal lat, final BigDecimal lon) {
     ...
     * 根据二进制和范围解码
     */
    private static BigDecimal decode(final BitSet bs, BigDecimal floor, BigDecimal ceiling) {
     ...
     * 根据经纬度和范围，获取对应二进制
     */
    private static BitSet getBitSet(final BigDecimal lat, BigDecimal floor, BigDecimal ceiling) {
        BitSet buffer = new BitSet(NUM_BITS);
        for (int i = 0; i < NUM_BITS; i++) {
            BigDecimal mid = middle(floor, ceiling);
            if (MathHelper.nvl(lat).compareTo(mid) >= 0) {
                buffer.set(i);
                floor = mid;
            } else {
                ceiling = mid;
     ...
     * 获取经纬度的 base32字符串
     */
    private static String getGeoHashBase32(final BigDecimal lat, final BigDecimal lng) {
     ...
     * 将五位二进制转化为 base32
     */
    private static char getBase32Char(final boolean[] base32) {
     ...
     * 获取坐标的geo二进制字符串
     */
    private static boolean[] getGeoBinary(final BigDecimal lat, final BigDecimal lng) {
     ...
     * 合并经纬度二进制
     */
    private static boolean[] merge(final boolean[] latArray, final boolean[] lngArray) {
